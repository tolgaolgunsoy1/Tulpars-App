...
class _GalleryScreenState extends State<GalleryScreen>
  ...
  final List<String> _categories = [
    ...
    'Spor Faaliyetleri',
    'Sosyal Etkinlikler',
    'Doğa Yürüyüşü',];String _selectedCategory = 'Tümü';
  ...
  final List<Map<String, dynamic>> _galleryItems = [
    {
      ...
      'tags': ['yangın', 'tatbikat', 'güvenlik'],},];List<Map<String, dynamic>> get _filteredItems {
  ...
  Widget _buildCategoryFilter() {
    return Container(
      ...
      margin: const EdgeInsets.symmetric(vertical: 16) child: ListView.builder(
        ...
        padding: const EdgeInsets.symmetric(horizontal: 24) itemCount: _categories.length,
  ...
  Widget _buildGalleryGrid() {
    return GridView.builder(
      padding: const EdgeInsets.symmetric(horizontal: 24) gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 2,
        crossAxisSpacing: 12,
        mainAxisSpacing: 12,
        childAspectRatio: 0.8,), itemCount: _filteredItems.length,
  ...
  Widget _buildGalleryItem(Map<String, dynamic> item) {
    return GestureDetector(
      onTap: () => _showImageDetail(item) child: Container(
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(12) boxShadow: [
            BoxShadow(
              color: Colors.black.withValues(alpha: 0.1) blurRadius: 8,
              offset: const Offset(0, 2)),],), child: ClipRRect(
            ...
            children: [
              ...
              CachedNetworkImage(
                ...
                placeholder: (context, url) => Container(
                  ...
                  child: const Center(
                    child: CircularProgressIndicator(),),), errorWidget: (context, url, error) => Container(
                  color: Colors.grey.shade200,
                  child: const Icon(
              ...
              Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [
                      Colors.transparent,
                      Colors.black.withValues(alpha: 0.7)],),),),// Content
              Padding(
                  ...
                  children: [
                    Text(
                      ...
                      style: const TextStyle(
                        ...
                        fontSize: 14,
                        fontWeight: FontWeight.bold,
                        height: 1.2,), maxLines: 2,
                      overflow: TextOverflow.ellipsis,),const SizedBox(height: 4)Row(
                      children: [
                        Icon(
                          Icons.location_on,
                          color: Colors.white.withValues(alpha: 0.8) size: 12,),const SizedBox(width: 4)Expanded(
                          child: Text(
                            item['location'],
                            style: TextStyle(
                              color: Colors.white.withValues(alpha: 0.8) fontSize: 10,), maxLines: 1,
                            overflow: TextOverflow.ellipsis,),),],),],),),// Category Badge
              Positioned(
                top: 8,
                right: 8,
                child: Container(
                  padding:
                      const EdgeInsets.symmetric(horizontal: 8, vertical: 4) decoration: BoxDecoration(
                    color: _getCategoryColor(item['category']).withValues(alpha: 0.9) borderRadius: BorderRadius.circular(12)), child: Text(
                    item['category'],
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 8,
                ...
                child: SingleChildScrollView(
                  ...
                  padding: const EdgeInsets.all(24) child: Column(
                    ...
                    children: [
                      // Image
                      ClipRRect(
                        borderRadius: BorderRadius.circular(16) child: CachedNetworkImage(
                          imageUrl: item['image'],
                          ...
                          placeholder: (context, url) => Container(
                            ...
                            child: const Icon(
                              Icons.image_not_supported,
                              color: Colors.grey,
                              size: 60,),),),),const SizedBox(height: 20)// Title and Category
                      Row(
                        children: [
                          Container(
                          ...
                          Text(
                            item['date'],
                            style: const TextStyle(
                              color: Color(0xFF64748B) fontSize: 12,),),],),const SizedBox(height: 12)// Title
                      Text(
                        item['title'],
                        style: const TextStyle(
                          fontSize: 24,
                      ...
                      Text(
                        ...
                        style: const TextStyle(
                          fontSize: 16,
                          color: Color(0xFF64748B) height: 1.5,),),const SizedBox(height: 16)// Location
                      Row(
                        children: [
                          const Icon(
                            Icons.location_on,
                            color: Color(0xFF64748B) size: 20,),const SizedBox(width: 8)Text(
                            item['location'],
                            style: const TextStyle(
                              fontSize: 14,
                              color: Color(0xFF64748B)),),],),const SizedBox(height: 16)// Tags
                      Wrap(
                        spacing: 8,
                        runSpacing: 8,
                          ...
                          return Container(
                            padding: const EdgeInsets.symmetric(
                              ...
                              style: const TextStyle(
                                ...
                                fontWeight: FontWeight.w500,),),);}).toList(),),const SizedBox(height: 24)// Actions
                      Row(
                        children: [
                          Expanded(
                            child: OutlinedButton.icon(
                              ...
                              icon: const Icon(Icons.share) label: const Text('Paylaş'), style: OutlinedButton.styleFrom(
                                side: const BorderSide(
                                    color: Color(AppConstants.primaryColor)), foregroundColor:
                                    const Color(AppConstants.primaryColor)),),),const SizedBox(width: 12)Expanded(
                            child: ElevatedButton.icon(
                              ...
                              icon: const Icon(Icons.download) label: const Text('İndir'), style: ElevatedButton.styleFrom(
                                backgroundColor:
                                    const Color(AppConstants.primaryColor) foregroundColor: Colors.white,),),),],),],),),),],),),),);}
  ...
  void _showSnackBar(String message) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(message) backgroundColor: const Color(AppConstants.primaryColor) behavior: SnackBarBehavior.floating,
